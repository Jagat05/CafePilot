...
>(({ className, ...props }, ref) => (
    <ToastPrimitives.Viewport
        ref={ref}
        className={cn(
            "fixed top-0 right-0 z-[100] flex max-h-screen w-full flex-col p-4 md:max-w-[420px] print:hidden",
            className
        )}
        {...props}
    />
...
>(({ className, ...props }, ref) => (
    <ToastPrimitives.Close
        ref={ref}
        className={cn(
            "absolute right-2 top-2 rounded-md p-1 opacity-100 transition-opacity focus:outline-none focus:ring-2 text-white hover:text-black hover:bg-white/20",
            className
        )}
        toast-close=""
        {...props}
